...
func TestHighlightSubstring(t *testing.T) {
	result := highlightSubstring("Hello World Hello", "hello")
	...
	result = highlightSubstring("all good", "missing")
	if result != "all good" {
...
func TestHighlightRegex(t *testing.T) {
	result := highlightRegex("error42 and error99", "error[0-9]+")
	...
	result = highlightRegex("error.*stuff", "[invalid(")
	if result == "" {
		t.Error("invalid regex fallback should not return empty")
	}
	// Non-matching should return unchanged.
	result = highlightRegex("all good", "error[0-9]+")
	if result != "all good" {
